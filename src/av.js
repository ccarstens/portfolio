import { store } from './store'
import { event } from './event'

export default {
    store,
    event,
    mediaElements: null,
    toInitialize: [],
    initializedElements: [],
    audioTestElement: null,

    init() {
        this.checkBrowserAutoplay()

        document.addEventListener('DOMContentLoaded', () => {
            this.mediaElements = document.querySelectorAll('audio, video')
            setTimeout(() => {
                this.loadMedia()
                this.loadImages()
            }, 500)
        })
    },
    initPendingMedia() {
        if (this.toInitialize.length > 0) {
            let tmp = this.toInitialize.slice()
            this.toInitialize.length = 0

            for (let el of tmp) {
                const elVol = el.volume
                el.volume = 0
                el.play().then(() => {
                    el.pause()
                        // el.currentTime = 0

                    el.volume = elVol
                    el.dataset.init = 'true'
                    this.initializedElements.push(el)
                    this.store.commit('SET_MIN_ONE_MEDIA_ELEMENT_INIT', true)
                })
            }
        }
    },
    loadMedia() {
        this.mediaElements.forEach((element) => {
            const source = element.getElementsByTagName('source')[0]
            const canPlayhandler = (event) => {
                if (this.store.state.can_autoplay_audio) {
                    element.dataset.init = 'true'
                    this.initializedElements.push(element)
                } else {
                    if (element.dataset.init === undefined) {
                        this.toInitialize.push(element)
                    }
                }
                element.oncanplay = null
            }
            element.addEventListener('canplay', canPlayhandler)

            this.event.$emit('av-hooked-element-to-canplay', element, {
                once: true,
            })

            source.src = source.dataset.src
            element.load()
        })
    },
    loadImages() {
        const images = document.querySelectorAll('img')
        for (let i = 0; i < images.length; i++) {
            const img = images[i]
            img.src = img.dataset.src
        }
    },

    /**
     * Check whether the browser can autoplay audio and video without a explicit user interaction
     */
    checkBrowserAutoplay() {
        const mp3 =
            'data:audio/mpeg;base64,'
        const ogg =
            'data:audio/ogg;base64,'

        try {
            if (this.audioTestElement === null)
                this.audioTestElement = new Audio()
            this.audioTestElement.src = null
            let src = this.audioTestElement.canPlayType('audio/ogg') ? ogg : mp3
            this.audioTestElement.autoplay = true

            this.audioTestElement.addEventListener('play', () => {
                this.store.commit('SET_CAN_AUTOPLAY_AUDIO', true)
            })

            this.audioTestElement.src = src
        } catch (e) {
            console.log('ERROR NO AUTO PLAY')
        }
    },
}