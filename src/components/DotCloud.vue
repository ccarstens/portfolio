<template>
    <div style="user-select: none" @mouseout="mouseOut" @mouseover="mouseOver">
        ...
        <vue-p5
            ...
            @keypressed.once="handleUserInteraction"
...
export default {
    ...
    methods: {
        ...
        draw(s) {
            if (
                this.deadParticlesCount === this.particles.length &&
                !this.backgroundMode
            ) {
        ...
        steerParticlesTowardsTarget() {
            this.particles.forEach((particle) => {
                ...
                if (this.backgroundMode) {
                    let factor =
                        this.target.copy().sub(particle.location).mag() * 0.01
        ...
        setTarget() {
            if (this.backgroundPolarMode) {
                this.targetTheta += 10
                const x =
                    this.targetRadius * this.s.sin(this.targetThetaInRadians)
                const y =
                    this.targetRadius * this.s.cos(this.targetThetaInRadians)
                ...
                if (this.target === null)
                    this.target = this.s.createVector(0, 0)
